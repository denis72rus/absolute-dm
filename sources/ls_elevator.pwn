...
#include <a_samp>
#include "../sources/core/color.inc"

#define ELEVATOR_SPEED     (5.0)   // Скорость движения лифта. (можно поставить свою)
#define DOORS_SPEED        (5.0)   // Скорость открывания дверей.
#define ELEVATOR_WAIT_TIME (5000)  // Время в мс, которое лифт будет ждать на каждом этаже, потом продолжать дверей
...
public OnObjectMoved(objectid)
{
	...
	if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
	{
	    ...
	    Label_Elevator	= Create3DTextLabel("{CCCCCC}Нажмите {FFFFFF}~k~~CONVERSATION_NO~{CCCCCC} для использования лифта", 0xCCCCCCAA, 1784.9822, -1302.0426, z - 0.9, 4.0, 0, 1);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_ID)
    {
        ...
        if(FloorRequestedBy[listitem] != INVALID_PLAYER_ID || IsFloorInQueue(listitem))
            SendClientMessage(playerid,CGRAY,"Этаж уже в очереди");
		else if(DidPlayerRequestElevator(playerid))
		    SendClientMessage(playerid,CGRAY,"Ты уже вызвал лифт");
		else
...
{
	if(!IsPlayerInAnyVehicle(playerid) && (newkeys & KEY_NO || newkeys == KEY_ANALOG_LEFT))
	{
	    new Float:pos[3];
		...
		else    // Is he in a floor button?
		{
		    if(pos[1] > -1301.4 && pos[1] < -1299.1447 && pos[0] < 1785.6147 && pos[0] > 1781.9902)
		    {
				...
				if(i <= 19)
				{
					...
					SendClientMessage(playerid,CLIGHTGREEN,"Лифт вызван");
					return 1;
				}
		    }
		}
...
stock Elevator_Initialize()
{
	...
	Label_Elevator          = Create3DTextLabel("{CCCCCC}Нажмите {FFFFFF}~k~~CONVERSATION_NO~{CCCCCC} для использования лифтаы", 0xCCCCCCAA, 1784.9822, -1302.0426, 13.6491, 4.0, 0, 1);
	...
	for(new i; i < sizeof(Obj_FloorDoors); i ++)
	{
		...
		format(string, sizeof(string), "{CCCCCC}%s\n{CCCCCC}Нажмите {FFFFFF}~k~~CONVERSATION_NO~{CCCCCC} для вызова лифта", FloorNames[i]);
...
stock ShowElevatorDialog(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_ID, DIALOG_STYLE_LIST, "{"#COLOR_DIALOG_MENU"}Лифт", string, "Выбрать", "Выход");
